...
import { Sparkles, Save, Loader2, HelpCircle, Shield, Mail, Check, Copy, Inbox, Settings2, Bot, Server, ShieldCheck } from 'lucide-react'
...
export default function AiSettings({ userEmail, userId }: AiSettingsProps) {
  ...
  const forwarderEmail = `${userId}@inbound.normaflow.hu`
        ...
        <div className="relative z-10 mt-6">
          {activeTab === 'automation' ? (
            <div className="space-y-6">
              ...
              {/* EmailConnectionCard Ingress Configuration */}
              <div className="border-t border-slate-800/80 pt-6">
                <EmailConnectionCard userEmail={userEmail} />
              </div>
            </div>
          ) : (
            <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-indigo-500/20 bg-indigo-950/20 p-6 shadow-2xl backdrop-blur-md">
        ...
        <div className="mt-6 space-y-4">
          <p className="text-sm text-slate-300 leading-relaxed">
            Az AI kétféle módon tudja fogadni a beérkező leveleit. Használhatja a közvetlen, valós idejű Cloudflare e-mail továbbítást az alábbi egyedi e-mail címre, vagy beállíthat rendszeres IMAP lekérdezést a meglévő vállalati fiókjához.
          </p>
...
export function EmailConnectionCard({ userEmail }: { userEmail: string }) {
  ...
  // Load existing configuration on mount/update
  useEffect(() => {
    if (!userEmail) return
    const docRef = doc(db, `users/${userEmail}/email_connections`, 'imap')
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data()
        setEmail(data.email || '')
        setImapHost(data.imap_host || 'imap.gmail.com')
        setImapPort(Number(data.imap_port || 993))
        // Password is encrypted on Firestore, so we leave password empty
        setPassword('')
      }
    }, (err) => {
      console.error('Error loading IMAP connection:', err)
    })
    return () => unsubscribe()
  }, [userEmail])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const trimmedHost = imapHost.trim()
    
    if (!trimmedEmail || !password || !trimmedHost || !imapPort) {
    ...
    try {
      ...
      if (!token) {
        throw new Error('Felhasználó nem azonosítható (hiányzó munkamenet token)')
      }

      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/configureEmailConnection`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Nem sikerült menteni a kapcsolódási adatokat.')
      }

      setSuccess('Az IMAP postafiók sikeresen szinkronizálva és titkosítva!')
      setPassword('') // Clear the password field for security
    } catch (err: any) {
      console.error('Error configuring email connection:', err)
      setError(err.message || 'Kapcsolódási hiba történt.')
    } finally {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-2xl backdrop-blur-md">
      ...
      <div className="flex items-center gap-3 border-b border-slate-800/80 pb-4">
        ...
        <div>
          <h3 className="text-lg font-bold text-white">Bejövő IMAP Postafiók Beállítása</h3>
          <p className="text-xs text-slate-400">Biztonságos kapcsolat létrehozása a vállalati postafiók olvasásához</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider font-mono">E-mail cím</label>
            <input
              type="email"
              required
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              placeholder="szamlak@iroda.hu"
              className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-950/70 p-3 text-xs text-slate-200 placeholder-slate-600 focus:border-violet-500 focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider font-mono">Alkalmazásjelszó / App Password</label>
            <input
              type="password"
              required
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="16 karakteres Google kód"
              className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-950/70 p-3 text-xs text-slate-200 placeholder-slate-600 focus:border-violet-500 focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider font-mono">IMAP Szerver</label>
            <input
              type="text"
              required
              value={imapHost}
          ...
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider font-mono">IMAP Port</label>
            <input
              type="number"
              required
              value={imapPort}
        ...
        <div className="flex items-center justify-between border-t border-slate-800/80 pt-4 mt-2">
          <div className="flex items-center gap-1.5 text-[10px] text-slate-400">
            ...
            Titkosított tárolás (Symmetric AES-256-GCM)
          </div>
          <button
            type="submit"
